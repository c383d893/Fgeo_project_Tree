...
system.time({master.list <- foreach(z = 1:null.itt, .combine='c', .multicombine=TRUE)%dopar%{    # we don't care about binding order, .combine= how to combine returned obj. ; %do% is not parallel                                                            # for each plotyear
  itt.list<-list()
  for(k in site) {
    site.list <- list()
...
saveRDS(master.list, 'data/Rand_GLMmodeloutputs_m1-5_10.21.rds')

print('GLM Null models complete')
